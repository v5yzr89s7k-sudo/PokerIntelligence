"""
V0.17 blocked quantitative evidence retention contract.
...
def main():
    ...
    #
    # The already-settled HJ evidence from frame 10 must now be
    # recoverable/reconciled WITHOUT requiring a new physical
    # stack observation from HJ.
    #
    # Production implementation may expose this under a different
    # internal representation, but FrameHandObserver needs one
    # deterministic reconciliation boundary.
    # --------------------------------------------------------
    ...
    assert hasattr(
        ...
        "MISSING ARCHITECTURE: blocked physical evidence has "
        "no deterministic reconciliation boundary"
    )

    # Production now reconciles automatically when UTG's fold
    # advances the chronology frontier. The primitive remains
    # callable, but exactly-once ownership means there is no work
    # left for a subsequent manual reconciliation.
    emitted = observer.reconcile_pending_evidence()
    ...
    again = observer.reconcile_pending_evidence()
