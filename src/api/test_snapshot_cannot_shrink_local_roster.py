        ...
        with TemporaryDirectory() as tmp:
            ...
            state = sm.handle_table_snapshot(
                state,
                {
                    ...
                    "dealt_in_seats": six,
                    "dealer_button_seat": "seat_mid_right",
                    # Deliberately wrong six-handed map.
                    "positions": {
                        "seat_upper_right": "HJ",
                        "seat_mid_right": "BTN",
                        "seat_lower_right": "SB",
            ...
            assert "seat_mid_left" in by_seat
            assert by_seat["seat_mid_left"]["stack_bb"] == 17.85

            canonical = sm.CANONICAL_STORE.load()

            assert len(canonical.players) == 7
            assert canonical.dealt_in_seats == seven
            # Position topology is state-machine structural state.
            assert state["positions"] == positions
            assert state["positions"]["seat_mid_left"] == "UTG+1"
            assert state["positions"]["seat_upper_left"] == "LJ"

            # Canonical player records must retain the same positions after
            # snapshot enrichment.
            assert canonical.players["seat_mid_left"].position == "UTG+1"
            assert canonical.players["seat_upper_left"].position == "LJ"
            ...
            print(
                "PASS snapshot cannot shrink local roster: "
                "7-player table_context survives incomplete "
                "6-player asynchronous snapshot"
            )
